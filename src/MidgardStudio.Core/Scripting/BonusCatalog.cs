...
public static class BonusCatalog
{
    ...
    public static readonly IReadOnlyList<BonusDefinition> All = new List<BonusDefinition>
    {
        ...
        B(CHpSp, "bMaxAP", "Max AP + n", "Flat increase to maximum AP (the 4th-job Activity Points resource).", Num("amount", "50")),
        B(CHpSp, "bMaxAPrate", "Max AP + n%", "Increases maximum AP by a percentage.", Pct("percent")),
        ...
        B(CAtk, "bPAtk", "P.Atk + n", "Physical Attack: the 4th-job power sub-stat (raises physical damage).", Num("amount")),
        B(CAtk, "bPAtkRate", "P.Atk + n%", "Physical Attack by percentage (4th-job sub-stat).", Pct("percent")),
        B(CAtk, "bSMatk", "S.MATK + n", "Spell MATK: the 4th-job magic sub-stat (raises magic damage).", Num("amount")),
        B(CAtk, "bSMatkRate", "S.MATK + n%", "Spell MATK by percentage (4th-job sub-stat).", Pct("percent")),
        ...
        B(CDef, "bResRate", "RES + n%", "Physical status resistance by percentage (4th-job stat).", Pct("percent")),
        B(CDef, "bMRes", "MRES + n", "Status resistance vs magic (4th-job stat).", Num("amount")),
        B(CDef, "bMResRate", "MRES + n%", "Magic status resistance by percentage (4th-job stat).", Pct("percent")),
        ...
        B(CAcc, "bCRate", "C.Rate + n", "Critical Rate: the 4th-job crit sub-stat.", Num("amount")),
        B(CAcc, "bCRateRate", "C.Rate + n%", "Critical Rate by percentage (4th-job sub-stat).", Pct("percent")),
        ...
        B(CSkill, "bHPlus", "H.Plus + n", "Heal Plus: the 4th-job sub-stat that boosts the HP you restore when healing.", Num("amount")),
        B(CSkill, "bHPlusRate", "H.Plus + n%", "Heal Plus by percentage (4th-job sub-stat).", Pct("percent")),
